...
def topic_llm_ollama(text_to_classify, model = model_name , url = "http://localhost:8000/api/generate"):
    ...
    TÂCHE : Associe l'extrait ci-dessous à **un seul** sujet parmi les catégories listées.
    CATEGORIES :
    • Allocations : prestations et aides liées au chômage, y compris les réformes de l’assurance chômage.
    • Réforme du droit du travail : modifications structurelles du marché de l’emploi et des contrats de travail.
    • Mesures sociales : politiques sociales, protections et dispositifs visant le bien-être des salariés.
    • Conditions de travail : environnement, sécurité et qualité de vie au travail.
    • Formation : développement des compétences et programmes de formation professionnelle.
    • Conflits sociaux et relations avec les syndicats : négociations, conflits collectifs, dialogue social et licenciements liés aux relations sociales.
    • Croissance et compétitivité : enjeux économiques affectant l’emploi et la performance du marché du travail.
    • Autres : sujets divers ne relevant pas des catégories précédentes.
    EXTRAIT : {text_to_classify}
    RÉPONSE : <UNIQUEMENT LE TITRE DU TOPIC, AUCUNE AUTRE EXPLICATION>"""
    data = {
        "model": model,
        "prompt": prompt,
        "stream": False}
...
#temp["result"].iloc[0]
#temp["sentence_text"].iloc[0]
#temp.iloc[15]["sentence_text"]
...
apply_in_batches(df, col_input = "sentence_text", col_output = "classification_topic", f = classify_all, other_col_to_save = ["solr_id", "sentence_id", "min_sentence_id", "max_sentence_id"],
                 n_batches = 10, file_name = "classification_topic", path_to_store_bashes = 'data/temp/bash/')
