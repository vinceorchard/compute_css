...
TÂCHE : Identifie le THEME PRINCIPAL en 1 à 10 mots maximum
EXTRAIT : {text_to_classify}
RÉPONSE (JSON uniquement, sans autres textes) : {{"topic": "THEME PRINCIPAL"}}"""
    data = {
        "model": model,
        "prompt": prompt,
        "stream": False}
...
apply_in_batches(df, col_input = "sentence_text", col_output = "classification_theme", f = classify_all, other_col_to_save = ["solr_id", "sentence_id", "min_sentence_id", "max_sentence_id"],
                 n_batches = 10, file_name = "classification_theme", path_to_store_bashes = 'data/temp/bash/')
